...
/***/ (function(module, exports, __webpack_require__) {
	...
	new Vue({
		...
		data: {
			...
			editUserUrl: 'auth/user/editUser'
		},
		components: {
		...
		methods: {
			...
			cropSuccess: function cropSuccess(data, field, key) {
                if (data.hasError) {
                    ...
                    $("[name=avatar]").attr("src", data.data);
                }
			},
            ...
            editUser: function () {
                var vm = this;
                var data = $("#editUser").serializeJSON();
                if (data.nickName === "") {
                    alert("昵称不能为空");
                    return;
                }
                post(vm.editUserUrl, JSON.stringify(data), function (result) {
                    if (!result.hasError) {
                        alert("保存成功");
                    }
                });
        ...
        mounted: function () {
			this.headers.YYYQUSERTOKEN=$.cookie('YYYQUSERTOKEN');
            $('[name=sex]').selectpicker().selectpicker('val', $("#hiddenSex").val());
        }
	});
